...
export async function sendMessageToConversation(
  ...
  const attempt = async (phone: string): Promise<string> => {
    ...
    if (messageType === 'template') {
      // For now, map template component building logic (legacy or new) to provider if possible, 
      // but CloudAPIProvider expects raw Meta API components for now, so we need to either build them here 
      // or within the provider. sendTemplateMessage handles legacy params internally, but IMessagingProvider 
      // signature uses components. Let's use the provider.
      // Wait, CloudAPIProvider's `sendTemplate` currently does not do the `buildSendComponents`.
      // Let's just use the provider to send the template. We can pass the raw template to CloudAPIProvider for now by keeping meta-api inside it.
    ...
    if (isMediaKind) {
      const result = await provider.sendMedia(
        ...
        { link: mediaUrl!, filename: filename || undefined },
