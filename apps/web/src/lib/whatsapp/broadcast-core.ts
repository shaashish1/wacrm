...
export async function createBroadcast(
  ...
  // We no longer decrypt the access token here, the provider will handle it.
...
export async function deliverBroadcast(
  ...
  let sentCount = 0;
  const provider = await providerFactory(plan.accountId);
    ...
    for (const variant of variants) {
      try {
        const result = await provider.sendTemplate(
          ...
          {
            templateRow: plan.templateRow ?? undefined,
