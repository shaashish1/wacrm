...
export async function engineSendText(
  ...
  // Config decryption removed, provider handles it

  const attempt = async (phone: string): Promise<string> => {
    const provider = await providerFactory(args.accountId);
    const r = await provider.sendText(args.accountId, phone, args.text);
    return r.messageId
  }
...
export async function engineSendMedia(
  ...
  const attempt = async (phone: string): Promise<string> => {
    ...
    const r = await provider.sendMedia(
      ...
      phone,
      args.kind,
      { link: args.link, filename: args.filename },
      args.caption
    );
    return r.messageId
  }
...
async function sendInteractiveViaMeta(
  ...
  const attempt = async (phone: string): Promise<string> => {
    const provider = await providerFactory(input.accountId);
