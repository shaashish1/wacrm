...
import { WWebJSConfig } from './wwebjs-config';
import {
  Accordion,
  AccordionItem,
  AccordionTrigger,
...
export function WhatsAppConfig() {
  ...
  const [providerType, setProviderType] = useState<'meta' | 'wwebjs'>('meta');
  ...
  const fetchConfig = useCallback(async (acctId: string) => {
    ...
    try {
      ...
      // Always call this — even without a whatsapp_config row — so
      // that provider_type (stored on the accounts table) is loaded.
      // Without this, accounts using the "wwebjs" provider would
      ...
      try {
        ...
        if (payload.provider_type) {
          detectedProvider = payload.provider_type;
          setProviderType(payload.provider_type);
        }
      } catch (err) {
      ...
      // Auto-detect: if the accounts table doesn't explicitly say
      // 'wwebjs' yet but a Baileys session row exists (worker
      // created it), switch the UI to the QR panel automatically.
      if (detectedProvider === 'meta') {
        const { data: session } = await supabase
          ...
          .eq('account_id', acctId)
          .maybeSingle();
        if (session?.provider_type === 'wwebjs') {
          setProviderType('wwebjs');
        }
      }
    } catch (err) {
  ...
  async function handleSave() {
    if (providerType === 'meta' && !phoneNumberId.trim()) {
      toast.error('Phone Number ID is required');
      return;
    }
    if (providerType === 'meta' && !config && (!accessToken.trim() || !tokenEdited)) {
    ...
    try {
      ...
      if (providerType === 'meta' && tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        payload.access_token = accessToken.trim();
      } else if (providerType === 'meta' && config) {
      ...
      if (providerType === 'wwebjs') {
        payload.access_token = 'dummy'; // satisfy API requirement minimally for wwebjs
        payload.phone_number_id = 'dummy';
      }
      ...
      if (data.registered === false && data.registration_error) {
        ...
        setPin('');
      } else if (data.provider_type === 'wwebjs') {
        toast.success('Provider set to WhatsApp Web (WWebJS).');
      } else {
        toast.success(
          ...
          <CardContent>
            <RadioGroup
              ...
              onValueChange={async (val) => {
                const newType = val as 'meta' | 'wwebjs';
                setProviderType(newType);
                // Persist immediately so the backend knows & reload
                // picks up the right panel without a manual save.
                try {
                  await fetch('/api/whatsapp/config', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ provider_type: newType }),
                  });
                } catch {
                  // Best-effort — the local state already switched.
                }
              }}
            ...
            >
              <div>
                ...
                >
                  ...
                  <span className="mt-1 text-xs text-muted-foreground">Official, high volume</span>
                </Label>
              </div>
              <div>
                <RadioGroupItem
                ...
                <Label
                  ...
                  className="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-transparent p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary"
                >
                  <span className="font-semibold text-foreground">WhatsApp Web</span>
                  <span className="mt-1 text-xs text-muted-foreground">QR Code scan, free</span>
                </Label>
              </div>
            </RadioGroup>
          </CardContent>
