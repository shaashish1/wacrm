...
export async function GET() {
  try {
    ...
    let providerType = accountData?.provider_type || 'meta';

    // Auto-detect: if accounts.provider_type isn't explicitly set but
    // a Baileys session row exists, treat the account as wwebjs.
    if (providerType === 'meta') {
      const { data: sessionRow } = await supabase
        ...
        .eq('account_id', accountId)
        .maybeSingle()
      if (sessionRow?.provider_type === 'wwebjs') {
        providerType = 'wwebjs';
      }
    }
...
export async function POST(request: Request) {
  try {
    ...
    const { phone_number_id, waba_id, access_token, verify_token, pin, provider_type, pairing_phone, start_session } = body
    ...
    if (provider_type) {
      ...
      if (accountUpdateError) {
        console.error('Failed to update provider_type', accountUpdateError);
      }
    }
    ...
    if (provider_type === 'wwebjs') {
       ...
       const shouldStart = start_session === true || !!pairing_phone
       if (shouldStart) {
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            'This WhatsApp phone number is already linked to another account on this instance. Each phone number can only be connected to one AudienceGate user.',
        },
        { status: 409 }
      )
    }
